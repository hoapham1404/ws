...
const defaultRoutes = [
  ...
  { name: "Screen of Death XP", path: "/screen-of-death-xp", image: DeathXPIcon },
  { name: "Screen of Death 10", path: "/blue-screen-of-death-windows", image: Death10Icon },
    ...
    <div className="flex justify-center mb-4 cursor-pointer overflow-x-auto whitespace-nowrap">
      {defaultRoutes.map((route) => (
        <div key={route.name} className={`flex flex-col items-center p-6 ${route.path === currentPath ? 'bg-gray-300' : ''}`}>
          ...
          >
            <Image src={route.image} alt={route.name} className="mx-auto object-cover" />
          </button>
