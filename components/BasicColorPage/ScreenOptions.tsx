...
import TemperatureSlider from "@/components/BasicColorPage/TemperatureSlider";
      ...
      <div className="flex flex-wrap justify-center mb-4 cursor-pointer ">
        {routes
          ...
          .map((option) => (
            <div
              key={option.name}
              className={`flex flex-col items-center p-6 ${option.path === currentPath ? "bg-gray-300" : ""}`}
            >
              <button
              ...
              ></button>
              <span className="mt-2 text-sm md:text-base underline">
                {option.name}
              </span>
            </div>
          ))}
